...
export default function PoDocuments({
  ...
  async function handleFile(file: File) {
    ...
    const path = `po-docs/${poId}/${Date.now()}-${safeName}`;
  ...
  async function remove(docId: string) {
    ...
    await fetch(`/api/purchase-orders/${poId}/documents?docId=${docId}`, {
      method: "DELETE",
    });
    load();
  }
  ...
  return (
    <div className="card">
      ...
      <p className="mb-4 text-xs text-slate-500">
        Customer PO, delivery slip, consignment note — attach here so everything
        for {poNumber} is auditable. PDF, Excel or image, up to {MAX_MB} MB.
      </p>
